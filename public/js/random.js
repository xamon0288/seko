...
const mainCard = document.querySelector('.swiper-slide-block-random-text');
const mainImg = document.querySelector('.swiper-slide-block-random-img');
let objectIdsUrl;
const artUrl =
	'https://collectionapi.metmuseum.org/public/collection/v1/objects/';
export let objectIds = [];
export let artArr = [];
let artUrlArr = [];
...
const randVal = () => {
	let random = departments[Math.floor(Math.random() * departments.length)];
	if (randomValue === 0) {
		objectIdsUrl = `https://collectionapi.metmuseum.org/public/collection/v1/objects?departmentIds=${random}`;
		console.log(objectIdsUrl);
	} else if (randomValue === random) {
		randVal();
	}
};
...
const getIds = async url => {
	...
	objectIds = ids.objectIDs.sort(() => 0.5 - Math.random()).slice(0, 1);
};
...
const loadFunc = async url => {
	...
	if (artArr.length < currIndex + 1) {
		...
		// sessionStorage.setItem('artRandom', JSON.stringify(artArr));
		// sessionStorage.setItem('indexRandom', JSON.stringify(currIndex));
	}
	render();
...
window.onload = async function () {
	// if (sessionStorage.artRandom) {
	// sessionStorage.setItem('artRandom', '');
	// sessionStorage.setItem('indexRandom', '');
	// 	await loadFunc();
	// } else {
	await loadFunc();
	// }
